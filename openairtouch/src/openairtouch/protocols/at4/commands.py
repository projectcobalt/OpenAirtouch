"""AirTouch 4 command intent mapping."""
...
from ..base import ProtocolCommandError
...
def _build_command_spec(action: str, data: dict[str, Any]) -> commands.CommandSpec:
    try:
        ...
        raise ProtocolCommandError(str(exc)) from exc

    raise ProtocolCommandError(f"unsupported command action: {action}")


def _int(data: dict[str, Any], key: str) -> int:
    if key not in data:
        raise ProtocolCommandError(f"missing required field: {key}")
    value = data[key]
    if isinstance(value, bool):
        raise ProtocolCommandError(f"{key} must be an integer")
    try:
        return int(value, 0) if isinstance(value, str) else int(value)
    except (TypeError, ValueError) as exc:
        raise ProtocolCommandError(f"{key} must be an integer") from exc
...
def _optional_number(data: dict[str, Any], key: str) -> float | None:
    ...
    value = data[key]
    if isinstance(value, bool):
        raise ProtocolCommandError(f"{key} must be a number")
    try:
        return float(value)
    except (TypeError, ValueError) as exc:
        raise ProtocolCommandError(f"{key} must be a number") from exc
...
def _coerce_bool(value: Any, key: str) -> bool:
    ...
    if isinstance(value, str):
        ...
        if lowered in {"false", "off", "no", "0"}:
            return False
    raise ProtocolCommandError(f"{key} must be a boolean")
...
def _int_list(data: dict[str, Any], key: str) -> list[int]:
    ...
    if not isinstance(value, list):
        raise ProtocolCommandError(f"{key} must be a list")
...
def _validate_smart_limits(action: str, data: dict[str, Any], state: dict[str, Any]) -> None:
    ...
    if not minimum <= setpoint <= maximum:
        raise ProtocolCommandError(f"setpoint must be between {minimum} and {maximum} for AC {ac + 1}, got {setpoint}")
...
def _record_list(data: dict[str, Any], key: str) -> list[dict[str, Any]]:
    ...
    if not isinstance(value, list) or any(not isinstance(item, dict) for item in value):
        raise ProtocolCommandError(f"{key} must be a list of objects")
    return value
...
def _hex_bytes(data: dict[str, Any], key: str, *, default: bytes | None = None) -> bytes:
    ...
    if isinstance(value, str):
        ...
        if len(compact) % 2:
            raise ProtocolCommandError(f"{key} must contain whole hex bytes")
        return bytes.fromhex(compact)
    raise ProtocolCommandError(f"{key} must be hex text or a byte list")
